...
sld = uicontrol('Style','slider','Min',0,'Max',50,'Value',6,'Units','normal','Position',[0.0137 0.0249 0.1050 0.0257],'Callback',@setUmbral);
txt = uicontrol('Style','text','Units','normal','Position',[0.0118 0.0597 0.0940 0.0257],'String','Threshold (6 sigma)');
Cs{3} = uicontrol('Style','pushbutton','String','Write results','Units','normal','Position',[0.1254 0.0174 0.0584 0.0480],'Callback',@writeSignifs,'Visible','off');
...
%%
    function visualizeChromosomes(cromosom,variables,signifs,props)
        ...
        % Se dibuja el umbral
        crms=cat(2,cromosom{~isnumeric});
        sigma=std(crms); umbral=nsigma*sigma;
        hold on; yyaxis left; plot(1:length(crms),repmat(umbral,1,length(crms)),'--','Color',[230,126,0]/255); plot(1:length(crms),-repmat(umbral,1,length(crms)),'--','Color',[230,126,0]/255);
        Cs{3}.Visible='on';
    end

%%
    function setUmbral(source,~)
        nsigma=source.Value;
        umbral=nsigma*sigma; txt.String=sprintf('Threshold (%.1f sigma)',nsigma);
        delete(findobj(gca,'LineStyle','--'))
        hold on; yyaxis left; plot(1:length(crms),repmat(umbral,1,length(crms)),'--','Color',[230,126,0]/255); plot(1:length(crms),-repmat(umbral,1,length(crms)),'--','Color',[230,126,0]/255);
    end
    function writeSignifs(~,~)
        names=variables(~isnumeric); names=cat(2,names{:});
        signfnames=names(abs(crms)>=umbral); vals=crms(abs(crms)>=umbral);
        for isn=1:length(signfnames)

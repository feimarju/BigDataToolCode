function showChromosomes_v3(countings_G1_all,countings_G2_all,groupNames,varNames,significativas)

cromosom={}; variables={}; countvar=0; signifs=[];
for iv=1:length(varNames)
    if ~isempty(varNames{iv})
        if ~iscell(varNames{iv})
            if length(groupNames{iv})==1 % Numerical var
...
vnames=cat(2,variables{:}); xlabel('Analyzed categories and numerical variables','Interpreter','Latex'); yyaxis left; ylabel('$\Delta p$ (categorical/datetime/string vars)','Interpreter','Latex'); yyaxis right; ylabel('$\Delta mean$, $\Delta std$ (numerical vars)','Interpreter','Latex'); set(gca,'FontSize',16,'FontName','Times')
...
if sum(ylim1-ylim2)>1e-12; ylim([ylim1(2)*ratio ylim1(2)]); end

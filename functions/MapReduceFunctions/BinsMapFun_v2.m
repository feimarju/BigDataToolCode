function BinsMapFun_v2(data,~,intermKVStore,var,subsetter,numericFiltering,trsFnc,thrVarName,threshold,thrFnc)
...
vals = subset.(var); if isthr; vth = subset.(thrVarName); end
if iscell(vals)
...
else; intermVals = {min(vals), max(vals), iqr(vals), length(vals), mean1, mean2};
end
